...
class NeuralNetwork:
    ...
    def __init__(self, 
        ...
        :param lmbd: The regularization coefficient used in the regularization term of the loss function. It controls the strength of the regularization and helps prevent overfitting
        :param regularization: Regularization is a technique used to prevent overfitting in machine learning models. It involves adding a penalty term to the loss function during training, which encourages the model to learn simpler patterns that generalize better to new data. The regularization coefficient (lmbd) controls the strength of the penalty term, and the, defaults to l2 (optional)
    ...
    def backward_pass(self, X, y):
        ...
        if self.regularization == 'l2':
            dW2 += (self.lmbd/m) * self.W2
            dW1 += (self.lmbd/m) * self.W1
        elif self.regularization == 'l1':
            dW2 += (self.lmbd/m) * np.sign(self.W2)
            dW1 += (self.lmbd/m) * np.sign(self.W1)
        elif self.regularization == 'elasticnet':
            dW2 += (self.lmbd/m) * np.sign(self.W2) + (self.lmbd/m) * self.W2
            dW1 += (self.lmbd/m) * np.sign(self.W1) + (self.lmbd/m) * self.W1
